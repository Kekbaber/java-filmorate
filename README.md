...
- `reviews` - отзывы пользователей на фильмы: содержание, признак полезности (is_positive), автор, фильм.
- `review_likes` - реакция на отзывы (like/dislike): пара `review_id`, `user_id` - первичный ключ. На основе реакций 
вычисляется рейтинг полезности отзыва (`useful`)
...
- Индексы на `friendships(user_id)` и `friendships(friend_id)` для быстрого поиска друзей.
